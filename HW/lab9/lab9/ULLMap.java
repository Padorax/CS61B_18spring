...
public class ULLMap<K, V>  implements Map61B<K, V> {
    ...
    public void put(K key, V val) {
        if (list != null) {
            ...
            if (lookup == null) {
                ...
                lookup.val = val;//changing value of existing key does not add to size
            }
        } else {
            list = new Entry(key, val, list);
            size = size + 1;
     ...
     *  in the dictionary. */
    private class Entry {
        //the newest key inserted in at the front of Entry
    ...
    private class ULLMapIter implements Iterator<K> {
        ...
        @Override
        public K next() {//move forward a step in next()
            K ret = cur.key;
            cur = cur.next;
            return ret;
        }
