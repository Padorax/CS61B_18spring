...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        validateSinkSwimArg(index);//no return

        /** TODO: Your code here. */
        int parent = parentIndex(index);
        //while (parent > 0 && (contents[index].item().compareTo(contents[parent].item()) < 0)) {
        while(parent > 0 && contents[index].priority() < contents[parent].priority()) {
            swap(index, parent);
            index = parent;
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);//can move this outside of sink
        /** TODO: Your code here. */
        ...
        if (leftindex == size) {
            if (contents[leftindex].priority() < contents[index].priority()) {
        ...
        if (contents[rightindex].priority() < contents[leftindex].priority()) minChildIndex = rightindex;

        if (contents[minChildIndex].priority() < contents[index].priority()) {
    ...
    public void insert(T item, double priority) {
        ...
        //need to increase size before swim as swim checks the validity of size
        swim(size);
    ...
    public T removeMin() {
        /* TODO: Your code here! */
        //note the outer class: ArrayHeap can access private variable(myItem) in its nested class: Node
        if (size == 0) return null;
        //T rootItem = contents[1].myItem;
        T rootItem = peek();
        swap(1, size);
        size -= 1;//must decrease size BEFORE sink
        sink(1);
        return rootItem;
    }
    ...
    public void changePriority(T item, double priority) {
        /* TODO: Your code here! */
        /* can not get the i when break
        for (int i = 0; i <= size; i++) {
            if (contents[i].myItem.equals(item)) {//item should be object, check class inside equals
                break;
            }
        }
        */
        ...
        if (contents[i].myPriority < priority) sink(i);
        if (contents[i].myPriority > priority) swim(i);
        return;
    }
